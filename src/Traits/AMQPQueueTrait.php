...
namespace Burning\PhpRabbitmq\Traits;
...
trait AMQPQueueTrait {
     ...
     * @var null
     */
    private $handlerCallbackMessage = null;
    ...
    public function pushRaw($payload, $routing_key, $exchange = null, array $options = []){
        try {
            ...
            // 重置回调函数处理结果日志
            $this->setHandlerCallbackMessage();
    ...
    public function ack_handler_callback(){
        return function(AMQPMessage $msg){
            ...
            $ack = sprintf('[ack_handler_callback] AMQPMessage[%s] ack, with content:%s, correlation_id:%s' . PHP_EOL,
                $msg->getDeliveryTag(),
                $msg->body,
                $msg->has('correlation_id') ? $msg->get('correlation_id') : '0'
            );

            $this->handlerCallbackMessage = $ack;
//            Log::info($ack);
        };
    }
    ...
    public function nack_handler_callback($message){
        return function(AMQPMessage $msg) use ($message){
            ...
            $nack = sprintf('[nack_handler_callback] AMQPMessage[%s] nack, with content:%s' . PHP_EOL,
                $msg->getDeliveryTag(),
                $msg->body
            );

            $this->handlerCallbackMessage = $nack;
//            Log::info($nack);
        };
    }
    ...
    public function return_listen_callback($message){
        return function($reply_code, $reply_text, $exchange, $routing_key, AMQPMessage $msg) use ($message){
            ...
            $return = sprintf('[return_listen_callback] AMQPMessage enqueue failed, with content:%s, reply_code:%s, reply_text:%s, exchange:%s, routing_key:%s' . PHP_EOL,
                $msg->getBody(),
                $reply_code,
                $reply_text,
                $exchange,
            ...
            $this->handlerCallbackMessage = $return;
//            Log::error($return);
        };
    }
     ...
     * @return null | String
     */
    public function getHandlerCallbackMessage()
    {
        return $this->handlerCallbackMessage;
    }
     ...
     * @return null | String
     */
    public function setHandlerCallbackMessage()
    {
        $this->handlerCallbackMessage = null;
    }
}
